...
************************************************************/
struct CKVertexBufferDesc : public CKRasterizerObjectDesc {
...
************************************************************/
struct CKIndexBufferDesc : public CKRasterizerObjectDesc {
	CKDWORD	m_Flags;		 // CKRST_VBFLAGS
...
//			Check();
		...
		if (!c->Next) {
			...
			if (prev) m_MaxContigous = prev->Size;
			else m_MaxContigous = 0;
		}
		// Found remove it
		if (prev) prev->Next = c->Next;
		else	 m_FreeCells = c->Next;
		c->Next = NULL;
		m_FreeSize -= c->Size;
...
*/
	void ClearAllCells() {
		if (!m_FreeCells) m_FreeCells = new FreeCell;
		else
...
************************************************************/
struct CKSharedVBDesc : public CKRasterizerObjectDesc {
	CKDWORD				m_Flags;		// CKRST_VBFLAGS
	CKDWORD				m_VertexFormat;	// Vertex format : CKRST_VERTEXFORMAT
	...
	virtual ~CKSharedVBDesc() {
		if (m_VB) m_VB->RemoveShared(m_Offset,m_VertexCount);
	}
};
...
************************************************************/
struct CKSharedIBDesc : public CKRasterizerObjectDesc {
	CKDWORD				m_Flags;		// CKRST_VBFLAGS
	...
	virtual ~CKSharedIBDesc() {
		if (m_IB) m_IB->RemoveShared(m_Offset,m_IndexCount);
	}
};
