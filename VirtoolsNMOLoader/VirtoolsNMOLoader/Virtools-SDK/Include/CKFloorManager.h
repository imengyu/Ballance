...
Arguments:
iPosition:a vector in world space to be tested.
oFloor:address of a pointer that will be filled by the function to point on the nearest floor
oFaceIndex:index of the floor face intersecting the vertical to iPosition
oNormal:normal of the floor at the intersection with the vertical to iPosition
oDistance: vertical distance between iPosition and the floor
iExcludeFloor: if given, floor to exclude from the detection
Return Value:
CKFLOOR_NOFLOOR if no floor is available,
CKFLOOR_DOWN if the down floor is the nearest (or only)
