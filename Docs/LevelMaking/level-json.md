...
```js
{
  ...
  "level": {
    ...
    //自定义天空盒预设，B 后 F 前 L 左 R 右 T 上 D 下
    "customSkyBox": {
      //图片资源的引入参考下方说明。
...
如果你要使用自定义天空盒，或者自定义背景音乐，您可以选择以下两种方式：

* 打包自定义图片或者音乐至当前关卡中。
* 直接加载模组包中的资源。

如果选择打包自定义图片或者音乐至当前关卡中，必须先在关卡中链接自己的图片音乐，这样才能被正确打包。

![图片](39.jpg)

* 打开关卡主 Prefab，找到 GameLevelResourcesLinker，在其中添加你的图片或者音乐，保存即可。
* 在 Level.json 中只需要写文件名即可。

如果选择直接加载模组包中的资源，您需要修改资源名称，资源名称的格式为 (包名):(资源路径)，例如：core.assets.skys:Sky_M_Back.bmp 是加载 core.assets.skys 模块下的 “Sky_M_Back.bmp” 资源文件。

为了安全起见，不允许加载网络图片或者本地图片。
