  ...
  public class PhysicsObject : MonoBehaviour
  {
    ...
    public void Physicalize()
    {
      if (Handle == IntPtr.Zero)
      {
        ...
        {
          Debug.LogWarning("Surface already exist");
        ...
        if (!doNotCreateSurface)
        {
          foreach (var mesh in m_Convex)
            pConvexs.Add(PhysicsApi.API.create_polyhedron(mesh.vertices, mesh.triangles));
          foreach (var mesh in m_Concave)
            pConcavies.Add(PhysicsApi.API.create_polyhedron(mesh.vertices, mesh.triangles));
        }
