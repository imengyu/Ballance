...
namespace BallancePhysics.Editor
{
    static class PhysicsBoundsHandleUtility
    {
        ...
        public static void CalculateCornerHorizon(float3 cornerPosition, float3x3 axes, float3 cornerNormal, float3 cameraCenter, float3 cameraForward, bool cameraOrtho, float radius, out Corner corner)
        {
            ...
            {
                angle           = new float3(
                                        BallancePhysics.MathUtils.Angle(axes[0], axes[1]),
                                        BallancePhysics.MathUtils.Angle(axes[1], axes[2]),
                                        BallancePhysics.MathUtils.Angle(axes[2], axes[0])
                                    ),
            ...
            for (int n = 0, sign = -1; n < 2; n++, sign += 2)
            {
                for (int i = 0; i < 3; i++)
                {
                    ...
                    var Q = MathUtils.Angle(cameraForward, axis1);
                    ...
                    vectorToPointOnHorizon = math.normalize(MathUtils.ProjectOnPlane(vectorToPointOnHorizon, axis1));

                    var intersectionDirection = vectorToPointOnHorizon;
                    var angle1 = MathUtils.SignedAngle(axis2, intersectionDirection, axis1);
                    var angle2 = MathUtils.SignedAngle(axis3, intersectionDirection, axis1);
        ...
        public static void DrawCorner(Corner corner, bool3 showAxis)
        {
            ...
            {
                var angleLength = MathUtils.SignedAngle(MathUtils.ProjectOnPlane(intersections[0], corner.cameraForward),
                                                   MathUtils.ProjectOnPlane(intersections[1], corner.cameraForward), corner.cameraForward);
                ...
                for (int A = 1, B = 2, C = 0; C < 3; A = B, B = C, C++)
                {
                    ...
                    {
                        if (!axesBackfaced[A]) { angleLength = MathUtils.Angle(intersections[0], axes[A]); axesBackfaced[A] = (angleLength < kDegreeEpsilon || angleLength > corner.angle[C] - kDegreeEpsilon); }
                        if (!axesBackfaced[B]) { angleLength = MathUtils.Angle(intersections[1], axes[A]); axesBackfaced[B] = (angleLength < kDegreeEpsilon || angleLength > corner.angle[C] - kDegreeEpsilon); }
                    }
                    else
                    if (corner.splitAxis[0][C])
                    {
                        if (showAxis[C])
                        {
                            angleLength = MathUtils.Angle(intersections[0], axes[A]);
                    ...
                    //if (corner.splitAxis[1][C])
                    {
                        if (showAxis[C])
                        {
                            angleLength = MathUtils.Angle(intersections[1], axes[A]);
