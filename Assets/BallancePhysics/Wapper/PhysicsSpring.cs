...
namespace BallancePhysics.Wapper
{
  ...
  [LuaApiDescription("在两个对象之间创建一个物理弹簧")]
  public class PhysicsSpring : PhysicsComponent
  {
    ...
    [Tooltip("弹簧松弛时弹簧的长度。")]
    [LuaApiDescription("弹簧松弛时弹簧的长度。")]
    /// <summary>
    /// 弹簧松弛时弹簧的长度。
    /// </summary>
    public float length = 1;
    [LuaApiDescription("定义弹簧的硬度。刚性弹簧：应在[0.001f..1]范围内。0.001=非常软。1=非常难。正常弹簧：（单位：牛顿/米）。应在范围[0..infinite]内。0=非常软。10=硬。")]
    ...
    [LuaApiDescription("阻尼系数，包括弹簧旋转的阻尼。刚性弹簧：未实施；对刚性弹簧没有影响。正常弹簧：该系数有助于最小化弹簧的数量。")]
    ...
    [LuaApiDescription("如果为true，则创建的弹簧将是刚性弹簧。然而，刚性弹簧的工作原理与普通弹簧类似-弹簧常数应在[0.001f..1]范围内-不会出现积分器问题->非常稳定。-可以使其比真正的弹簧更硬。-阻尼应在[0..1]范围内")]
    ...
    [LuaApiDescription("Set this to TRUE, if spring values should be multiplied with the average virtual mass of both objects")]
    ...
    [LuaApiDescription("Set this to TRUE, if spring values should only be applied when the length exceeds spring_len")]
